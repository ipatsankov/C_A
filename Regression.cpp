	...
	for (int i = 0; i < Triang.Get_N(); i++)
	{
		if (Triang.Get_Triangle(i).Belongs_P(P) == 1)
		{
			...
			for (int j = 0; j < 3; j++)
			{
				...
				{
					Point PPP( PP.GetS(k));
					p.push_back((1 /( 10*d(PPP, P)))* (1 / (10 * d(PPP, P))));
...
double Regression::Evaluation(vector <Point>& All_Point)
{
	...
	for (int n = 0; n<Triang.Get_N(); n++) 
	{
		...
		/*for (int k = 0; k < P.size_S(); k++)
		{
			...
			Point PP(P.GetS(k));
			if(d(PP, P) != 0) p.push_back(1 / 10*(d(PP, P)));
		...
		for (int i = 1; i < 3; i++)
		{
			Point PP(Triang.Get_Triangle(n).Get_P(i));
			if (d(PP, P) != 0) p.push_back((1 / (10 * (d(PP, P))))* (1 / (10 * d(PP, P))));
			if (d(PP, P) == 0) p.push_back(10);
			S += PP.Get_F() * p[p.size() - 1];
			v += p[p.size() - 1];
		}
